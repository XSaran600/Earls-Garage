	...
	: depthTexHandle(0) 
{
	...
	memset(colourTexHandles, 0, sizeof(GLenum) * 16);
}
...
void FrameBufferObject::createFrameBuffer(unsigned int fboWidth, unsigned int fboHeight, unsigned int numColourBuffers, bool useDepth)
{
	...
	glGenTextures(numColourTex, colourTexHandles); // CHANGED
	...
	for (int i = 0; i < numColourTex; i++)
	{
		...
		// Here I am making each texture the same size, but you may not want to 
		...
		// Need to keep track of attachments added
	...
	glDrawBuffers(numColourTex, bufferAttachments); // CHANGED
